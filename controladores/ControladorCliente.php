...
if (!empty($funcion_autos)) {
    dameTablaDeAutos($funcion_autos);
}

function dameTablaDeAutos($cedula) {
    require_once "../conexion/conexion.php";
    $conexion = new Conexion();
    $conexion->iniciarSesion();
    $consulta = mysqli_query($conexion->dameConexion(), "SELECT ca.chapa, m.descripcion, mar.descripcion
															FROM clientes c
